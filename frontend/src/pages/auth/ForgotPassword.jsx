...
export default function ForgotPassword() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background:'#F6F8FB' }}>
      <div className="w-full max-w-md bg-white rounded-3xl shadow-blue overflow-hidden border border-teal-100">
          ...
          {sent ? (
            ...
            <>
              <div className="text-center mb-7">
                <div className="w-14 h-14 rounded-2xl mx-auto mb-4 flex items-center justify-center shadow-md" style={{ background:'linear-gradient(135deg,#0F766E,#D32F2F)' }}>
                  <MdDirectionsCar className="text-white w-7 h-7" />
                </div>
                <h1 className="text-2xl font-extrabold text-slate-900" style={{ fontFamily:'Space Grotesk,sans-serif' }}>Forgot Password</h1>
                ...
                <div className="flex p-1 rounded-xl bg-teal-50 border border-teal-100">
                  {['USER','ORGANIZER'].map(r => (
                    <button key={r} type="button" onClick={() => setRole(r)}
                      ...
                      style={{
                        background: role===r ? 'linear-gradient(135deg,#0F766E,#14B8A6)' : 'transparent',
